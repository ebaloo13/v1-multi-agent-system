...
export type OperationsRunArtifactV1 = {
  schema_version: 1;
  run_id: string;
  started_at: string;
  finished_at: string;
  status: OperationsRunArtifactStatus;
  exit_code: 0 | 1;
  model: string;
  operations_data_path: string;
  operations_data_sha256: string;
  ...
  validated_output?: OperationsOutput;
  validation_errors?: unknown;
  parse_error_message?: string;
  unexpected_message?: string;
};

export type OperationsRunEventLine = {
  ts: string;
  type: string;
  subtype?: string;
  summary?: string;
...
export function operationsRunDirFor(repoRoot: string, runId: string): string {
  return path.join(repoRoot, "artifacts", "runs", runId);
}

export function operationsEventLineFromSdkMessage(message: {
  type: string;
  subtype?: string;
...
export async function appendOperationsRunEvent(
  runDir: string,
