...
export type OrchestratorRunArtifactV1 = {
  schema_version: 1;
  run_id: string;
  started_at: string;
  finished_at: string;
  status: OrchestratorRunArtifactStatus;
  exit_code: 0 | 1;
  model: string;
  /** Logical input source (orchestrator consumes validated audit output, not a static JSON file). */
  orchestrator_input_source: string;
  orchestrator_data_path: string;
  orchestrator_data_sha256: string;
  ...
  /** Validated routing decision from the orchestrator model (e.g. on subagent_error). */
  orchestrator_routing_output?: OrchestratorOutput;
  validated_output?: OrchestratorFinalOutput;
  subagent_failure?: {
    agent: OrchestratorOutput["activated_agents"][number];
    code: string;
    message?: string;
  };
...
export type OrchestratorRunEventLine = {
  ts: string;
  type: string;
  subtype?: string;
  summary?: string;
...
export function orchestratorRunDirFor(repoRoot: string, runId: string): string {
  return path.join(repoRoot, "artifacts", "runs", runId);
}

export function orchestratorEventLineFromSdkMessage(message: {
  type: string;
  subtype?: string;
